...
const RE_SRC_FULL_OIG = /src="(https:\/\/th\.bing\.com\/th\/id\/OIG[^"]+)"/g;
const RE_SRC_REL_OIG = /src="(\/th\/id\/OIG[^"]+)"/g;
...
class BingCreate {
    ...
    parseImageUrls(html, modelValue) {
        ...
        if (modelValue === MODEL_MAP.GPT4O) {
            let m;
            const r1 = new RegExp(RE_SRC_FULL_OIG.source, 'g');
            const r2 = new RegExp(RE_SRC_REL_OIG.source, 'g');
            while ((m = r1.exec(html))) srcUrls.push(m[1]);
            if (!srcUrls.length) while ((m = r2.exec(html))) srcUrls.push(m[1]);
        } else {
            let m;
